...
var jsSHA = __webpack_require__(2);
...
var vm = new Vue({
    ...
    data: {
        ...
        numbersForAnimate: Array.from({ length: 49 }, function () {
            return Math.floor(Math.random() * 99 + 1);
        }), // set of random number for animate process
    ...
    computed: {
        hash: function hash() {
            // create hash from random
            var shaObj = new jsSHA("SHA-256", "TEXT");
            shaObj.update(this.random.toString());
            return shaObj.getHash("HEX");
        },
        validateBetAmount: function validateBetAmount() {
        ...
        errorText: function errorText() {
            ...
            if (!this.validateBetAmount) {
                return this.betAmount > this.balance ? 'Bet amount can`t be more than your balance' : 'Bet amount must be float number more than 0';
            }
            return '&nbsp;';
        }
    },
    mounted: function mounted() {
        // get saved player`s balance
    ...
    methods: {
        getFreeCredits: function getFreeCredits() {
            this.balance += 100;
            localStorage.setItem('balance', this.balance);
        },
        makeBet: function makeBet(direction) {
            var _this = this;
            ...
            this.win = null;
            var el = $('.win-animate');
            var height = $(el).height() * (49 / 50) * -1;
            Vue.set(this.numbersForAnimate, 49, this.random);
            $(el).animate({ top: height }, 1000, function () {
                Vue.set(_this.numbersForAnimate, 0, _this.random);
                $(el).css('top', 0);
                _this.inProcess = false;
                var summ = null;
                if (direction == 'Hi') {
                    summ = _this.random >= _this.number ? _this.betAmount * (_this.betHiPayout - 1) : null;
                } else {
                    summ = _this.random <= _this.number ? _this.betAmount * (_this.betLoPayout - 1) : null;
                }
                _this.win = summ ? true : false;
                _this.balance += summ ? summ : _this.betAmount * -1;
                localStorage.setItem('balance', _this.balance);
                _this.random = Math.round(Math.random() * 99) + 1;
            });
        }
    }
});
